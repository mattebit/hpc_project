...
int find_lightest_edge(int** graph, int node_id) {
    int i = 0;
    int min = (NODE_COUNT * NODE_COUNT * 4) + 100; // graph[node_id][0];
    int min_id = 1;
    for (i; i < NODE_COUNT; i++)
    {
...
int find_min_indx(int* nodes_id, int** graph) {
    int i = 0;
    int min = (NODE_COUNT * NODE_COUNT * 4) + 100; // graph[0][nodes_id[0]];
    int min_id = -1;
    for (i; i < NODE_COUNT; i++)
    {
        if (nodes_id[i] != -1)
...
void remove_recursive_references(int** min_graph) {
    int i = 0;
    for (i; i < NODE_COUNT; i++)
    {
        int j = 0;
...
int main(int argc, char** argv) {
    ...
    while (true)
    {
        ...
        //remove_recursive_references(min_graph);
